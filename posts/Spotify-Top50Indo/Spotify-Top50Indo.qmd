...
```{r}
mood_count <- audio_features %>%
  count(mood) %>%
  arrange(desc(n))

highchart() %>%
  hc_add_series(mood_count,
                "pie",
                hcaes(name = mood,
                      y = n),
                name = "Jumlah lagu") %>%
  hc_title(
    text = "Spotify Top 50 - Indonesia: Suasana Hati"
  ) %>%
  hc_subtitle(
    text = "Ini adalah pie chart yang memvisualisasikan mood seluruh lagu dalam playlist Spotify Top 50 - Indonesia"
  ) %>%
  hc_caption(
    text = "Nilai di atas diambil dari empat variabel Audio Features Spotify: Valence, Danceability, Acousticness, dan Energy",
    style = list(fontSize = "9px")
  ) %>%
  hc_credits(
    enabled = TRUE,
    text = "http://alifdwt.github.io"
  )
```
...
audio_features %>%
  ...
  hc_credits(enabled = TRUE, text = "http://alifdwt.github.io") %>%
